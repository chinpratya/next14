...
import { useListAssessmentSubmissionRespondentFormComment } from '../../../api/lis-assessment-submission-respondents-form-comment';

import { FormProgressMenu } from './form-progress-menu';

type FormProgressprops = {
  assessmentId: string;
  setDisComment: (disable: boolean) => void;
};
export const FormProgress = ({
  ...
  const { data } =
    useListAssessmentSubmissionRespondentFormComment(
      assessmentId
    );
  ...
  useEffect(() => {
    if (data && data.length > 0) {
      const findComment = _.find(
        data,
        (value) => value === selectedFormKey
      );

      setDisComment(
        findComment === undefined ? true : false
      );
    }
  }, [data, setDisComment, selectedFormKey]);
