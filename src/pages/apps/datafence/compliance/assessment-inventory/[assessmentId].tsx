...
const AssessmentInventoryDetailPage = () => {
  ...
  return (
    <FallbackError isError={isError}>
      <PageHeader
        title={
          <IntlMessage id="compliance.assessmentInventory.detail.title" />
        }
        subtitle={data?.data?.name}
        onBack={() =>
          router.replace(
            '/apps/datafence/compliance/assessment-inventory'
          )
        }
      />
      <TabsInside
        ...
        items={[
          ...
          },
          {
            key: 'webform',
            label: (
              <IntlMessage id="compliance.assessmentInventory.webForm" />
            ),
            children: (
              <FallbackError
