...
const PreviewPreferencePage = () => {
  ...
  const gdprCookieNotice = (
    ...
    try {
      ...
      const bannerNotice = document.querySelector(
        '.gdpr-cookie-notice-nav-item-settings'
      ) as HTMLElement | null;
      if (bannerNotice) {
        bannerNotice.click();
      }
      const cookieNoticeTextBox = document.querySelector(
        '.gdpr-cookie-notice-text-box'
      ) as HTMLElement | null;
      if (cookieNoticeTextBox) {
        cookieNoticeTextBox.style.display = 'none';
      }
    } catch (error) {
...
export default PreviewPreferencePage;
